...
//array-is-sorted/ 
//
// Related Topics 数组 双指针 二分查找 👍 57 👎 0


//leetcode submit region begin(Prohibit modification and deletion)
function twoSum(numbers: number[], target: number): number[] {
    if (numbers.length === 2) return [0, 1]
    // const nums = numbers.slice(0, binarySearch(numbers, target) + 1)
    for (const index in numbers) {
        const res = binarySearch(numbers, target - numbers[index])
        if (numbers[res] === target - numbers[index]) {
            return [Number(index), res]
        }
    }
}
