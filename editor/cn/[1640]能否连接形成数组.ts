...
function canFormArray(arr: number[], pieces: number[][]): boolean {
    ...
    for (const piece of pieces) {
        if (piece.length === 1) {
            if (!arr.includes(piece[0])) return flag = false
        } else {
            const index = arr.indexOf(piece[0])
            const temp = arr.slice(index, index + piece.length)
            for (let i = 0, len = piece.length; i < len; i++) {
                if (!(temp[i] && piece[i] === temp[i])) return flag = false
            }
        }
    }
    return flag
