...
function areNumbersAscending(s: string): boolean {
    const arr = s.split(' ').filter((value) => /^[1-9]*[0-9]*$/.test(value))
    for (let i = 1, len = arr.length; i < len; i++) {
        if (Number(arr[i]) - Number(arr[i - 1]) <= 0) return false
    }
    return true
}
...
console.log(areNumbersAscending('1 box has 3 blue 4 red 26 green and 12 yellow marbles'))
console.log(areNumbersAscending('hello world 5 x 5'))
console.log(areNumbersAscending('hello world 1 1 x 5'))
